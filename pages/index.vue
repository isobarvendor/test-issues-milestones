<template>
  <div id="main" class="home" v-if="CMSContent">
    <div class="wrapper">
      <MastheadVideo
        :data="CMSContent[0]"
        :isCountDown="!notCountDown"
        v-if="CMSContent[0].homepage.mastheadSection.video"
      />
      <Masthead :data="CMSContent[0]" :isCountDown="!notCountDown" v-else />

      <CampaignPeriod
        :data="configData.campaignPeriod"
        :howData="CMSContent[0].worksSection"
        v-if="configData"
      />
      <Prizes
        v-if="configData && notCountDown"
        :data="CMSContent[0].exclusivePrizes"
        :ngpsPrize="listPrizesData"
        :exclusivePrizes="
          configData ? configData.ExclusivePrizes.ExclusivePrizes : false
        "
        :winners="CMSContent[0].luckyWinner"
        :prize="CMSContent[0].prize"
      />
...
export default {
  ...
  mounted() {
    ...
    if (
      this.CMSContent &&
      new Date(this.CMSContent[0].endDate).getTime() > new Date().getTime()
    ) {
  ...
  methods: {
    ...
    isVideo(data) {
      return data && data.desktopImage.length > 0
        ? data.desktopImage[0].url.includes("mp4")
        : false;
    },
    getListWallet() {
      this.$store
    ...
    getListPrize() {
      ...
      for (let a = 0; a < luckyDraw.length; a++) {
        this.$store
          .dispatch(GET_LIST_PRIZE, luckyDraw[a].NPGS[0].configID)
          .then(response => {
            this.listPrizesData = [
              ...this.listPrizesData,
  ...
  computed: {
    ...
    campaignType() {
      return this.$store.getters.getCMSConfig
        ? this.$store.getters.getCMSConfig.campaignTypes.mechanicType
        : null;
    },
    getAttempt() {
      return this.$store.getters.getCMSConfig
        ? this.$store.getters.getCMSConfig.attempts
        : null;
    }
  }
};
